...
const EngagementSnapshot = ({ startDate, endDate, selectedGroup }) => {
  ...
  useEffect(() => {
    const fetchEngagement = async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/engagement?${queryParams.toString()}`);
        const json = await res.json();

        if (json.status === "ok") {
          setData(json);
        } else {
          console.warn("❌ Engagement fetch failed:", json.message);
          setData(null);
        }
      } catch (err) {
  ...
  // --- Label logic using real team names ---
  const isTeam =
    selectedGroup &&
    teams.map((t) => t.toLowerCase()).includes(selectedGroup.toLowerCase());
  const viewLabel = selectedGroup
    ? isTeam
      ? `Viewing Engagement for Team: ${selectedGroup}`
      : `Viewing Engagement for Member: ${selectedGroup}`
    : "Viewing Engagement for All Messages";
